...
- **Availability** (`Admin → Availability`): a schedule holds weekly hours in its own timezone plus date overrides (custom hours or a whole day off). A default Mon–Fri 9–17 schedule is created with the profile. Members can keep several schedules (say, "Working hours" and "Evenings"); a new one starts as a copy of the default, any of them can be made the default, and each event type picks the schedule it uses, falling back to the default when its schedule is deleted. Date overrides belong to a schedule; a day off can be applied to all of a member's schedules in one go (the default when adding one), shows as "all schedules", and is removed everywhere with one click.
- **Event types** (`Admin → Event types`): what people can book — duration, slot interval, buffers before/after, minimum notice, booking horizon, max bookings per day, one or more locations (Bookly video, Google Meet, Zoom, Teams, phone, in person, custom; with several, the attendee picks one when booking and the first is the default; a phone call asks the attendee for their number with the country preselected from their location, and an in-person meeting whose address is left blank asks the attendee where to meet; both appear in the invitation, emails and booking pages), booking questions, "requires confirmation", hidden (link-only), and **guests**: how many colleagues the attendee may bring (0 = none). Guests are listed on the booking, added to the calendar event, and get the invitation, reminders and any cancellation; only the person who booked can change or cancel.
...
Confirmation to the attendee (with `.ics`, `METHOD:REQUEST`), notification to the host, cancellation to both (`METHOD:CANCEL`), and reminders at each event type's offsets (default 24h and 1h before), scheduled precisely per booking through the job queue (pg-boss in-process, or QStash on serverless) with a `booking.reminders` sweep every 5 minutes as the safety net; without either, call `/api/cron/tick` with `Authorization: Bearer $CRON_SECRET` (the repo ships a GitHub Actions schedule in `.github/workflows/reminders.yml`, enabled with the `REMINDERS_ENABLED` repository variable). All go through `@bookly/email` (console / Resend / SMTP).
