...
- **Availability** (`Admin → Availability`): a schedule holds weekly hours in its own timezone plus date overrides (custom hours or a whole day off). A default Mon–Fri 9–17 schedule is created with the profile. Members can keep several schedules (say, "Working hours" and "Evenings"); a new one starts as a copy of the default, any of them can be made the default, and each event type picks the schedule it uses, falling back to the default when its schedule is deleted.
