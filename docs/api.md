...
answered. `guests` is a list of colleagues' emails, accepted up to the event type's `maxGuests`;
bookings expose them as `guests`. Keys are hashed at rest and shown once; revoke from the admin.
