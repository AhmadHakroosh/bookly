...
export const unsubscribeUrl = (base: string, contactId: string) =>
  `${base}/unsubscribe/${unsubscribeToken(contactId)}`;

/** Target of the List-Unsubscribe header: answers one-click (RFC 8058) POSTs. */
export const unsubscribePostUrl = (base: string, contactId: string) =>
  `${base}/api/unsubscribe/${unsubscribeToken(contactId)}`;
